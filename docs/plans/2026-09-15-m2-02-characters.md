...
**Goal:** Four characters as data + a cached `buildKartMesh` factory, replacing the hardcoded red kart, with identical handling for everyone.

**Architecture:** `characters.ts` holds the roster as `CharacterDef[]` (palette + silhouette multipliers, visual only) and `buildKartMesh(def)` — the M1 placeholder kart extracted from `main.ts`, parameterized by the def. Per Vision §11 (no per-frame allocations, shared geometries/materials) the module caches one geometry per shape, one material per color, and one `Group` per character id — repeated races reuse GPU resources instead of leaking them (Plan 04 re-adds the same group). `main.ts` builds `CHARACTERS[0]` until the select flow lands. Physics, `KartParams`, and `stepKart` are untouched: identical handling is structural (no per-character params exist).

**Tech Stack:** TypeScript (strict), vitest, Three.js.

**Spec:** `docs/SPEC-M2.md` §6 (roster), §10 (module structure + caching rule).
...
- Silhouette multipliers scale the mesh (shared unit geometry), never physics; wheels stay stock size.
- Geometries, materials, and kart groups are created once and cached — `buildKartMesh` returns the same `Group` instance for the same character id.
...
### Task 2: Cached `buildKartMesh` + reskins

**Files:**
- Modify: `src/game/characters.ts` (append; add the Three.js import at the top of the file)
...
- Produces: `function buildKartMesh(def: CharacterDef): THREE.Group` — named children `body` and `nose`, 4 wheels, 6 children total, **same instance per character id** (cached). Silhouettes are applied via `mesh.scale` over shared unit geometries; materials are cached per color. Used by Plan 04.
...
- [ ] **Step 3: Implement** (add `import * as THREE from 'three';` at the top of `src/game/characters.ts`, then append)
...
Then in `src/game/main.ts`: delete the inline `kartMesh`/`body`/`nose`/wheel block (the red kart) and replace with:
...
git commit -m "refactor: extract cached buildKartMesh with character reskins"
```
